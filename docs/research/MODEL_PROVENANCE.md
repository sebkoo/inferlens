...
|------|----------|--------|--------------------|-------|----------|
| Core ML | `MobileNetV2FP16.mlmodel` | `https://ml-assets.apple.com/coreml/models/Image/ImageClassification/MobileNetV2/MobileNetV2FP16.mlmodel` | FP16 (Apple's recommended baseline) | 12.4 MB | pinned at rung 05 |
| TFLite | `mobilenet_v2` float32 `.tflite` | Google-hosted (`download.tensorflow.org` / Kaggle Models) — exact URL verified at rung 05 | FP32 (Google's default float model) | ~11 MB | pinned at rung 05 |
...
- **Different weights.** Apple's and Google's MobileNetV2 are trained independently: same
  *architecture*, not the same *artifact*. Cross-model top-1 agreement is a measured,
  published result (rung 13), never an equality assertion (ADR-0003).
...
- **`make bootstrap` precedes `swift build`.** The models are fetched by script (the SPM
  ...
  must run first. CI (rung 27) runs bootstrap before test; the README states it in one
  line.
...
The exact Google `.tflite` download URL is verified when rung 05 runs. The **precision
