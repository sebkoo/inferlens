...
instructions are committed under `docs/prompts/` from the LiteRT engine onward, each without its
reading list and landing steps and each with a note on where it went wrong; earlier ones lived in
session handoffs and were never reconstructed.
...
**The TensorFlow Lite engine** ([prompt](prompts/litert-engine.md))
...
**The fallback chain** ([prompt](prompts/fallback-chain.md))
...
**Cancellation** ([prompt](prompts/cancellation.md))
...
**The signal and the export** ([prompt](prompts/signal-and-export.md))
...
**Continuous integration** ([prompt](prompts/ci.md))
...
**The app shell** ([prompt](prompts/app-shell.md))
...
**The label table** ([prompt](prompts/labels.md))
...
**The remote leg** ([prompt](prompts/remote-leg.md))
...
**The offline eval** ([prompt](prompts/offline-eval.md))
